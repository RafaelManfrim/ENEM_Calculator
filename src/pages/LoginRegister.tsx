...
import bgImg from "/mac.jpg"

const loginSchema = yup.object({
  email: yup.string().required('Digite o seu e-mail').email('E-mail inválido'),
  password: yup.string().required('Digite sua senha').min(6, 'A senha precisa ter no mínimo 6 caracteres')
})
...
export function LoginRegister() {
  ...
  return (
    <Center minH="100vh" w="full" bgImg={bgImg} bgPosition="center" bgSize="cover">
      <Container maxW="lg" py={['12', '24']} px={['2', '8']}>
        <Box py={['4', '8']} px={['4', '10']} bg='white' boxShadow='md' borderRadius='xl'>
          <Logo mb="6" />
          {pathname === '/login' && (
            <Stack as="form" spacing="6" onSubmit={loginForm.handleSubmit(handleLogin)}>
              <Stack spacing="5">
                <Input
                  id="email"
                  type="email"
              ...
              <HStack spacing="1" justify="center">
                ...
                <Button variant="link" colorScheme="orange" onClick={handleDontHaveAnAccount}>
                  Registre-se
                </Button>
              </HStack>
              <Button colorScheme="orange" type="submit">Entrar</Button>
            </Stack>
          )}

          {pathname === '/register' && (
            <Stack as="form" spacing="6" onSubmit={registerForm.handleSubmit(handleRegister)}>
              <Stack spacing="5">
                <Input
                  id="name"
                  type="text"
              ...
              <HStack spacing="1" justify="center">
                ...
                <Button variant="link" colorScheme="orange" onClick={handleAlreadyHaveAnAccount}>
                  Faça login
                </Button>
              </HStack>
              <Button colorScheme="orange" type="submit">Registrar</Button>
            </Stack>
          )}
        </Box>
      </Container>
